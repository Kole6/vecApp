		...
		>
			<swiper-item>
				<load-more ref="scroll" @onPullDown="onPullDown" @onLoadMore="onLoadMore" :styleObj="{ height: wrapperHeight }" :loadStatus="loadStatus1">
					<view class="wrapper">
						<view class="list-item" v-for="(item, index) in dataArr1" :key="index">
			...
			<swiper-item>
				<load-more ref="scroll" @onPullDown="onPullDown" @onLoadMore="onLoadMore" :styleObj="{ height: wrapperHeight }" :loadStatus="loadStatus2">
					<view class="wrapper">
						<view class="list-item" v-for="(item, index) in dataArr2" :key="index">
							<view class="flag" @click="handleListTaped(item)">
								<block v-if="item.hasSelected"><image src="/static/indexIcon/selected.png" mode="aspectFit" style="height: 36upx; width:36upx"></image></block>
								<block v-else><view class="selecting"></view></block>
							</view>

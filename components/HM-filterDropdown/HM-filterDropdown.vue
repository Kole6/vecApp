<template>
  <view class="HMfilterDropdown" :class="{ 'hm': hm}" @touchmove.stop.prevent="discard">
    <view class="nav">
        ...
        <block v-if="page.type=='hierarchy'&& page.submenu.length>0">
          ...
          <block v-for="(sub,index) in page.submenu" :key="index">
            <scroll-view
              ...
              :scroll-y="true"
              v-if="activeMenuArr[page_index][0]==index&&sub.submenu&&sub.submenu.length>0"
...
export default {
  ...
  methods: {
    initMenu() {
      ...
      //加载菜单数据
      console.log(
        "this.filterData.length: " + JSON.stringify(this.filterData.length)
      );
    ...
    selectHierarchyMenu(page_index, level1_index, level2_index, level3_index) {
      ...
      if (
        ...
        let sub = this.subData[page_index].submenu[level1_index].submenu[
          level2_index
        ];
    ...
    selectFilterLabel(page_index, box_index, label_index) {
      let find_index = this.activeMenuArr[page_index][box_index].indexOf(
        label_index
      );
      if (find_index > -1) {
